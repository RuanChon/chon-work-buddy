...
export default function Mistakes() {
  ...
  function del(id) {
    apply(d => {
      d.mistakes = d.mistakes.filter(x => x.id !== id)
    })
  }
    ...
    <div>
      <form onSubmit={submit} className="mistake-form">
        <div className="row">
          ...
          <button type="button" className="btn" onClick={voiceToText}>
            🎙️ 语音转文字
          </button>
          {recording ? (
            <button type="button" className="btn rec" onClick={stopRec}>
              ⏹ 停止 {recSec}s
            </button>
          ) : (
        ...
        <div className="thumbs">
          {images.map((im, i) => (
            <img key={i} src={im.url} alt="" className="thumb" />
          ))}
        </div>
        <div className="audios">
          {audios.map((a, i) => (
            <audio key={i} src={a.url} controls />
          ))}
        </div>
      ...
      <div className="row filter pd-10">
        <button className={filter === 'all' ? 'btn sm active' : 'btn sm'} onClick={() => setFilter('all')}>
          全部
        </button>
        {BOARDS.map(b => (
          <button
            key={b.key}
            className={filter === b.key ? 'btn sm active' : 'btn sm'}
      ...
      <div className="mistake-list pd-10">
        ...
        {list.map(m => {
          ...
          return (
            <div key={m.id} className="mistake-item">
              ...
              <div className="thumbs">
                {m.images?.map((im, i) => (
                  <img key={i} src={im.url} className="thumb" alt="" />
                ))}
              </div>
              <div className="audios">
                {m.audios?.map((a, i) => (
              ...
              <button className="btn ghost sm" onClick={() => del(m.id)}>
                删除
              </button>
            </div>
          )
