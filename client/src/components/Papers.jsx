...
export default function Papers() {
  ...
  function del(id) {
    apply((d) => { d.papers = d.papers.filter((x) => x.id !== id); });
  }
  ...
  return (
    <div>
      <div className="row filter">
        {PAPER_TYPES.map((t) => (
          <button key={t} className={type === t ? 'btn sm active' : 'btn sm'} onClick={() => setType(t)}>{t}</button>
        ))}
      </div>
      ...
      <form onSubmit={submit} className="paper-form">
        ...
        {sections.map((s, i) => (
          <div key={i} className="row sec-row">
            ...
            <input type="number" placeholder="得分" value={s.score} onChange={(e) => upSection(i, 'score', e.target.value)} />
            <input type="number" placeholder="满分" value={s.total} onChange={(e) => upSection(i, 'total', e.target.value)} />
            {sections.length > 1 && (
              <button type="button" className="btn ghost sm" onClick={() => setSections((a) => a.filter((_, j) => j !== i))}>×</button>
            )}
          </div>
        ))}
        <button type="button" className="btn sm" onClick={addSection}>＋ 增加板块</button>
        ...
        <button className="btn primary" type="submit">保存试卷</button>
      </form>

      <h3>{type} 成绩趋势（平均 {avg}）</h3>
      <Chart option={option} height={260} />

      <div className="paper-list">
        {list.length === 0 && <p className="muted">还没有{type}试卷</p>}
        {list.map((p) => (
          <div key={p.id} className="paper-item">
            <div className="pi-head">
              <b>{p.name}</b>
              <span className="muted">{p.date}</span>
              <span className="tag">{p.score}分 / 正确率{p.rate}%</span>
            </div>
            <div className="pi-secs">
              {p.sections.map((s) => <span key={s.name} className="chip">{s.name}:{s.score}/{s.total}</span>)}
            </div>
            {p.note && <p className="note">备注：{p.note}</p>}
            {p.mistakes && <p className="note">易错：{p.mistakes}</p>}
            <button className="btn ghost sm" onClick={() => del(p.id)}>删除</button>
          </div>
        ))}
      </div>
    </div>
